...
BEGIN
    ...
    -- Insert paket baru
    INSERT INTO paket (nomor_resi, nama_pengirim, nama_penerima, alamat_asal, alamat_tujuan, berat, layanan, status, tanggal_kirim, catatan)
    VALUES (p_nomor_resi, p_nama_pengirim, p_nama_penerima, p_alamat_asal, p_alamat_tujuan, p_berat, p_layanan, p_status, p_tanggal_kirim, p_catatan);
    
    -- Dapatkan ID paket yang baru ditambahkan
    SET v_paket_id = LAST_INSERT_ID();
    
    -- Tambahkan tracking history pertama
    INSERT INTO tracking_history (paket_id, tanggal_waktu, status_tracking, lokasi, keterangan)
    VALUES (v_paket_id, NOW(), CONCAT('Status: ', p_status), 'Gudang Asal', 'Paket baru ditambahkan ke sistem');
    
    -- Return paket_id
    SELECT v_paket_id AS paket_id, 'Paket berhasil ditambahkan' AS message;
END //
...
BEGIN
    ...
    SELECT id INTO v_paket_id FROM paket WHERE nomor_resi = p_nomor_resi;
    
    IF v_paket_id IS NOT NULL THEN
        -- Tambahkan tracking history
        INSERT INTO tracking_history (paket_id, tanggal_waktu, status_tracking, lokasi, keterangan)
        VALUES (v_paket_id, NOW(), p_status_tracking, p_lokasi, p_keterangan);
        
        SELECT 'Tracking berhasil ditambahkan' AS message;
    ELSE
        SELECT 'Paket tidak ditemukan' AS message;
    END IF;
END //
...
BEGIN
    DECLARE v_count INT;
    SELECT COUNT(*) INTO v_count FROM paket WHERE status = p_status;
    RETURN v_count;
END //
